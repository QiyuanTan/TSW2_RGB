...
ADR-002 selects Windows HID LampArray after foreground and packaged ambient per-key smoke tests passed on the reference keyboard. Background ownership transfer took 65.643 seconds, so temporary `IsAvailable=false` is a non-fatal waiting state. The backend must use `DeviceWatcher` and `AvailabilityChanged`, keep discovery and ownership asynchronous, resume rendering after acquisition, and release on shutdown. Normalized physical keys are translated to Windows virtual keys and then lamp indices only inside the adapter; core, profile, and rule code never stores those representations. Production backend issue 09 remains gated until issue #5 records the ambient soak, visible restoration, forced-exit, and reconnect checks.
